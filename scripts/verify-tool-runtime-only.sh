...
printf '%s\n' "$config" | grep -q 'AGENTDOCK_TOOL_RUNTIME_ONLY: "true"' || {
  echo "tool-runtime-only verification failed: AGENTDOCK_TOOL_RUNTIME_ONLY is not forced to true" >&2
  exit 1
}
...
printf '%s\n' "$config" | grep -q '^    build:' || {
  echo "tool-runtime-only verification failed: hardened profile is not building this fork" >&2
  exit 1
}
